...
export default function TimetablePage() {
    ...
    const handleSave = async (isPublic = false, customTitle?: string) => {
        ...
        setIsSaving(true);
        try {
            const title = isPublic ? 'Shared Timetable' : (customTitle || 'Untitled');
    ...
    const handleShare = async () => {
        if (!session?.user?.email) {
            showToast('Please sign in to share timetables.');
            return;
        }
        if (currentTT.length === 0) return;
        
        showToast('Generating shareable link...');
        try {
            ...
            showToast('An error occurred while sharing.');
        }
    };
    ...
    return (
        <div className="min-h-screen bg-[#F5E6D3] font-sans flex flex-col items-center justify-between">
            ...
            <div className="w-full max-w-[1400px] mx-auto bg-[#FFFBF0] rounded-[16px] p-3 shadow-sm mb-4">
